...
export class AboutTourComponent implements OnInit {
  ...
  constructor(
    ...
    this.http.get<any>('http://localhost:3000/post').subscribe((element) => {
      ...
      this.Thetourincludes.push(
        element[this.id - 1].Thetourincludes.includes('სასტუმროში განთავსება')
      );
      this.Thetourincludes.push(
        element[this.id - 1].Thetourincludes.includes('სალაშქრო აღჭურვილობა')
      );
      this.Thetourincludes.push(
        element[this.id - 1].Thetourincludes.includes('გიდის მომსახურება')
      );
      this.Thetourincludes.push(
        element[this.id - 1].Thetourincludes.includes('ფოტოგრაფის მომსახურება')
      );
      arrlist = obj;
      this.tourlist = obj;
